...
const std::wstring& system_error::error2str(unsigned long error)
{
	...
	BOOL ret = FormatMessage(
		...
		NULL, error, system_locale,(LPWSTR) &local_buf, 0, NULL);

	if (!ret) {
		HMODULE hnetmsg = LoadLibraryEx(L"netmsg.dll", NULL, DONT_RESOLVE_DLL_REFERENCES);
		if (hnetmsg != nullptr) {
			ret = FormatMessage(
				FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_ALLOCATE_BUFFER,
				hnetmsg, error, system_locale, (LPWSTR)&local_buf, 0, NULL);
